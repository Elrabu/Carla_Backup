...
def semantic_lidar_data(point_cloud_data):
    ...
    for detection in point_cloud_data:
        if detection.object_tag in value_list:
            ...
            vehicle_spectator = world.get_spectator()
            vehicle_spectator.set_transform(simulator_camera_location_rotation)


def car_control():
    actor_vehicle.apply_control(carla.VehicleControl(throttle=0.51))
    time.sleep(20)
...
try:
    ...
    car_model = get_blueprint_of_world.filter('vehicle.lincoln.mkz_2020')[0]
    ...
    '''
    print(actor_vehicle.get_transform())
